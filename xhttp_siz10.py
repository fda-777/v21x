...
def _resp_headers(fp: str) -> dict[str, str]:
...
async def _open_tcp_from_header(first_chunk: bytes) -> tuple[asyncio.StreamReader, asyncio.StreamWriter, str, int]:
    _, address, port, payload = await parse_vless_header(first_chunk)
...
async def _check_link(uuid: str) -> None:
...
async def _get_or_create_session(uuid: str, mode: str, session_id: str, ip: str = "نامشخص") -> SessionState:
    ...
    async with XHTTP_LOCK:
        ...
        sess: SessionState = {
            ...
            "down_q": asyncio.Queue[QueueItem](maxsize=DOWNLINK_QUEUE_MAX),
            "last_seen": time.time(),
...
async def _mark_real_mode(session_id: str, sess: SessionState, real_mode: str) -> None:
...
async def _pump_tcp_to_queue(session_id: str, uuid: str, reader: asyncio.StreamReader, down_q: asyncio.Queue[bytes | None]) -> None:
    first = True
...
async def _open_tcp_for_session(session_id: str, uuid: str, sess: SessionState, first_chunk: bytes) -> None:
...
def _downstream_gen(sess: SessionState) -> AsyncGenerator[bytes, None]:
