...
KeyboardButton: TypeAlias = dict[str, str]
KeyboardMarkup: TypeAlias = dict[str, list[list[KeyboardButton]]]
LinkData: TypeAlias = dict[str, Any]
WizardData: TypeAlias = dict[str, Any]

BOT_TOKEN = os.environ.get("TELEGRAM_BOT_TOKEN", "").strip()
_admin_ids_raw = os.environ.get("TELEGRAM_ADMIN_IDS", "").strip()
ADMIN_IDS: set[int] = {int(x) for x in _admin_ids_raw.replace(" ", "").split(",") if x.isdigit()} if _admin_ids_raw else set()
...
_running = False
_pending: dict[int, dict[str, Any]] = {}   # chat_id -> {"action": "wizard", "step": "...", "data": {...}}
...
def _parse_speed_text(text: str) -> int | None:
...
async def _call(method: str, **params: object) -> dict[str, Any] | None:
    if _client is None:
        return None
    try:
        payload = dict(params)
        r = await _client.post(f"{API_BASE}/{method}", json=payload, timeout=40)
        data = r.json()
        if not data.get("ok"):
...
async def _send(chat_id: int, text: str, kb: KeyboardMarkup | None = None) -> dict[str, Any] | None:
    payload: dict[str, object] = {"chat_id": chat_id, "text": text, "parse_mode": "HTML", "disable_web_page_preview": True}
    if kb:
        payload["reply_markup"] = kb
    return await _call("sendMessage", **payload)

async def _edit(chat_id: int, message_id: int | None, text: str, kb: KeyboardMarkup | None = None) -> None:
    if message_id is None:
        await _send(chat_id, text, kb)
        return
    payload: dict[str, object] = {"chat_id": chat_id, "message_id": message_id, "text": text, "parse_mode": "HTML", "disable_web_page_preview": True}
    if kb:
        payload["reply_markup"] = kb
...
async def _answer_cb(cb_id: str | None, text: str = "") -> None:
    if cb_id is None:
        return
...
# ── Keyboards ────────────────────────────────────────────────────────────────
def _main_menu_kb() -> KeyboardMarkup:
...
def _links_list_kb(page: int) -> KeyboardMarkup:
    items = sorted(LINKS.items(), key=lambda kv: str(kv[1].get("created_at", "")), reverse=True)
    total = len(items)
    ...
    for uid, l in chunk:
        ...
        rows.append([{"text": f"{dot} {str(l.get('label','?'))[:28]}", "callback_data": f"view:{uid}"}])
...
def _link_detail_kb(uid: str, active: bool) -> KeyboardMarkup:
    return {"inline_keyboard": [
...
def _confirm_delete_kb(uid: str) -> KeyboardMarkup:
    return {"inline_keyboard": [
...
def _wizard_skip_kb(step_key: str, label: str) -> KeyboardMarkup:
    return {"inline_keyboard": [
...
def _wizard_alpn_kb() -> KeyboardMarkup:
    return {"inline_keyboard": [
...
def _wizard_unlimited_kb(step_key: str) -> KeyboardMarkup:
    return _wizard_skip_kb(step_key, "♾ نامحدود")

def _wizard_confirm_kb() -> KeyboardMarkup:
    return {"inline_keyboard": [
        [{"text": "✅ ساخت کانفیگ", "callback_data": "w:confirm"}],
        [{"text": "❌ انصراف", "callback_data": "w:cancel"}],
    ]}

def _wizard_prompt(step: str, data: WizardData) -> str:
...
def _wizard_summary(data: WizardData) -> str:
    limit_bytes = data.get("limit_bytes")
    limit = "نامحدود" if not limit_bytes else fmt_bytes(int(limit_bytes))
    speed_limit_bytes = data.get("speed_limit_bytes")
    speed = "نامحدود" if not speed_limit_bytes else f"{int(speed_limit_bytes)*8/1024/1024:.1f} Mbps"
    iplim = data.get("ip_limit", 0) or "نامحدود"
    days = data.get("expires_days", 0)
    days_txt = "بدون انقضا" if not days else f"{int(days)} روز"
    proto = str(data.get("protocol", DEFAULT_PROTOCOL) or DEFAULT_PROTOCOL)
    alpn = str(data.get("alpn") or f"پیش‌فرض ({DEFAULT_ALPN_BY_PROTOCOL.get(proto, 'http/1.1')})")
    return (
...
# ── View builders ────────────────────────────────────────────────────────────
def _format_detail(uid: str, l: dict[str, Any]) -> str:
    status = "🟢 فعال" if is_link_allowed(l) else "🔴 غیرفعال/منقضی"
    limit_bytes = l.get("limit_bytes")
    limit = "نامحدود" if not limit_bytes else fmt_bytes(int(limit_bytes))
    speed_limit_bytes = l.get("speed_limit_bytes")
    speed = "نامحدود" if not speed_limit_bytes else f"{int(speed_limit_bytes)*8/1024/1024:.1f} Mbps"
    exp = l.get("expires_at")
    exp_txt = exp.split("T")[0] if isinstance(exp, str) and exp else "بدون انقضا"
    proto = str(l.get("protocol", DEFAULT_PROTOCOL) or DEFAULT_PROTOCOL)
    alpn = str(l.get("alpn") or f"پیش‌فرض ({DEFAULT_ALPN_BY_PROTOCOL.get(proto, 'http/1.1')})")
    return (
        f"<b>{l.get('label','?')}</b>\n"
        f"وضعیت: {status}\n"
...
async def _handle_message(msg: dict[str, Any]) -> None:
    ...
    chat = cast(LinkData | None, raw_chat if isinstance(raw_chat, dict) else None)
    chat_id_value = chat.get("id") if chat else None
    try:
    ...
    if pending and pending.get("action") == "wizard" and text:
        ...
        wizard_data = cast(WizardData, pending.get("data") if isinstance(pending.get("data"), dict) else {})
        pending["data"] = wizard_data

        if step == "label":
            wizard_data["label"] = text[:60] or "کانفیگ جدید"
            pending["step"] = "protocol"
            await _send(chat_id, _wizard_prompt("protocol", wizard_data), _wizard_protocol_kb())
            return
        ...
        if step == "alpn":
            wizard_data["alpn"] = text.strip()[:100]
            pending["step"] = "port"
            await _send(chat_id, _wizard_prompt("port", wizard_data), _wizard_skip_kb("port", f"⏭ پیش‌فرض ({DEFAULT_PORT})"))
            return
        ...
        if step == "port":
            ...
            wizard_data["port"] = p
            pending["step"] = "volume"
            await _send(chat_id, _wizard_prompt("volume", wizard_data), _wizard_unlimited_kb("volume"))
            return
        ...
        if step == "volume":
            ...
            if parsed is None:
                ...
                return
            wizard_data["limit_bytes"] = parsed
            pending["step"] = "speed"
            await _send(chat_id, _wizard_prompt("speed", wizard_data), _wizard_unlimited_kb("speed"))
            return
        ...
        if step == "speed":
            ...
            if parsed is None:
                ...
                return
            wizard_data["speed_limit_bytes"] = parsed
            pending["step"] = "iplimit"
            await _send(chat_id, _wizard_prompt("iplimit", wizard_data), _wizard_unlimited_kb("iplimit"))
            return
        ...
        if step == "iplimit":
            ...
            if n is None:
                ...
                return
            wizard_data["ip_limit"] = n
            pending["step"] = "days"
            await _send(chat_id, _wizard_prompt("days", wizard_data), _wizard_unlimited_kb("days"))
            return
        ...
        if step == "days":
            ...
            if n is None:
                ...
                return
            wizard_data["expires_days"] = n
            pending["step"] = "confirm"
            await _send(chat_id, _wizard_summary(wizard_data), _wizard_confirm_kb())
            return
...
async def _handle_callback(cb: dict[str, Any]) -> None:
    raw_message = cb.get("message")
    message = cast(LinkData | None, raw_message if isinstance(raw_message, dict) else None)
    raw_chat = message.get("chat") if message else None
    chat = cast(LinkData | None, raw_chat if isinstance(raw_chat, dict) else None)
    chat_id_value = chat.get("id") if chat else None
    try:
        chat_id = int(chat_id_value) if chat_id_value is not None else None
        ...
        chat_id = None
    message_id_value = message.get("message_id") if message else None
    try:
        message_id = int(message_id_value) if message_id_value is not None else None
    except (TypeError, ValueError):
        message_id = None
    data = str(cb.get("data") or "")
    cb_id = str(cb.get("id") or "") or None
    ...
    if data.startswith("w:"):
        ...
        step = str(pending.get("step") or "")
        wdata = cast(WizardData, pending.get("data") if isinstance(pending.get("data"), dict) else {})
        pending["data"] = wdata
        ...
        if data == "w:confirm" and step == "confirm":
            expires_days = int(wdata.get("expires_days", 0) or 0)
            expires_at = (datetime.now() + timedelta(days=expires_days)).isoformat() if expires_days > 0 else None
            uid, link = await make_link(
                label=str(wdata.get("label") or "کانفیگ جدید"),
                limit_bytes=int(wdata.get("limit_bytes", 0) or 0),
                expires_at=expires_at,
                protocol=str(wdata.get("protocol", DEFAULT_PROTOCOL) or DEFAULT_PROTOCOL),
                fingerprint=str(wdata.get("fingerprint", DEFAULT_FINGERPRINT) or DEFAULT_FINGERPRINT),
                alpn=str(wdata.get("alpn", "") or ""),
            ...
            _pending.pop(chat_id, None)
            await _edit(chat_id, message_id, f"✅ کانفیگ ساخته شد.\n\n{_format_detail(uid, link)}", _link_detail_kb(uid, bool(link.get("active", False))))
            return
    ...
    if data.startswith("view:"):
        ...
        if not l:
            ...
            return
        await _edit(chat_id, message_id, _format_detail(uid, l), _link_detail_kb(uid, bool(l.get("active", False))))
        return

    if data.startswith("toggle:"):
        uid = data.split(":", 1)[1]
        l = await set_link_active(uid, not bool(LINKS.get(uid, {}).get("active", True)))
        if not l:
            await _edit(chat_id, message_id, "این کانفیگ دیگه وجود نداره.", _main_menu_kb())
            return
        await _edit(chat_id, message_id, _format_detail(uid, l), _link_detail_kb(uid, bool(l.get("active", False))))
