...
if importlib.util.find_spec("aiofiles") is not None:
    ...
    # 'io' not needed; using built-in open in thread for async file shim
    ...
    class _AiofilesShim:
        @staticmethod
        def open(path: str, mode: str = 'r', encoding: Optional[str] = None):
            return _AsyncFileWrapper(path, mode, encoding)
...
try:
    ...
    raise ImportError(f"Required FastAPI dependencies not installed. Please install: pip install fastapi uvicorn httpx[http2]") from e
import logging
...
# ── In-memory state ───────────────────────────────────────────────────────────
LinkState = dict[str, Any]
ConnectionState = dict[str, Any]
StatsState = dict[str, int | float]
ErrorLogEntry = dict[str, Any]
ActivityLogEntry = dict[str, Any]
...
error_logs: deque[ErrorLogEntry] = deque(maxlen=50)
activity_logs: deque[ActivityLogEntry] = deque(maxlen=200)
hourly_traffic: defaultdict[str, int] = defaultdict(int)
...
AUTH: dict[str, str] = {"password_hash": hash_password(os.environ.get("ADMIN_PASSWORD", "X4GKING"))}
...
def get_host(request: Request | None = None) -> str:
    ...
    if request is not None:
        ...
        if h:
            ...
            return h
    host_value = os.environ.get("RAILWAY_PUBLIC_DOMAIN")
    return str(host_value or CONFIG["host"])
...
def vless_link_for_link(link: LinkState, uid: str, host: str) -> str:
...
def unique_ips_for_uuid(uuid: str) -> set[Any]:
...
async def subscription_single(uuid: str, request: Request):
    ...
    # Ensure label is a string before quoting to avoid type errors
    label = str(link.get("label", ""))
    return Response(content=content, media_type="text/plain",
                    headers={"profile-title": quote(label), "support-url": "https://t.me/X4GHUB"})
...
# ── Stats ─────────────────────────────────────────────────────────────────────
@app.get("/stats")
async def get_stats(_token: str = Depends(require_auth)) -> dict[str, Any]:
    async with LINKS_LOCK:
        snap = dict(LINKS)
    return {
...
async def get_connections(_token: str = Depends(require_auth)) -> dict[str, Any]:
    """
    ...
    for _conn_id, c in connections.items():
    ...
    for uid, cfg in by_uuid.items():
        ...
        ip_list.sort(key=lambda x: str(x.get("last_connected_at") or ""), reverse=True)
        configs.append({
            "uuid": uid,
            "label": cfg["label"],
    ...
    configs.sort(key=lambda x: str(x.get("last_connected_at") or ""), reverse=True)
...
async def set_link_active(uid: str, active: bool) -> LinkState | None:
    async with LINKS_LOCK:
        if uid not in LINKS:
            return None
...
# ── Link Management ───────────────────────────────────────────────────────────
@app.post("/api/links")
async def create_link(request: Request, _token: str = Depends(require_auth)) -> dict[str, Any]:
    body: dict[str, Any] = await request.json()
...
@app.get("/api/links")
async def list_links(request: Request, _token: str = Depends(require_auth)) -> dict[str, Any]:
    host = get_host(request)
    async with LINKS_LOCK:
        snap = dict(LINKS)
...
async def update_link(uid: str, request: Request, _token: str = Depends(require_auth)) -> dict[str, Any]:
    body: dict[str, Any] = await request.json()
    async with LINKS_LOCK:
        if uid not in LINKS:
...
async def delete_link(uid: str, _token: str = Depends(require_auth)) -> dict[str, Any]:
...
async def http_proxy(target_url: str, request: Request):
    ...
    try:
        ...
        headers: dict[str, str] = {k: v for k, v in request.headers.items() if k.lower() not in _HOP and k.lower() != "host"}
        method = request.method
        client = http_client
        if client is None:
            raise HTTPException(status_code=503, detail="proxy client unavailable")
...
if __name__ == "__main__":
    ...
    uvicorn.run("main:app", host="0.0.0.0", port=port, log_level="info", workers=1)  # type: ignore[reportUnknownMemberType]
